...
class KokoroEngine:
    ...
    def synthesize_stream(
        ...
        """
        Yields (graphemes, wav_bytes) for each chunk generated by KPipeline.
        """
        text = text.strip()
        if not text:
            return
        ...
        try:
            ...
            for gs, ps, audio in generator:
                if audio is not None and len(audio) > 0:
                    ...
                    if student_id is not None:
                        from tts.tts_quota import tts_quota
                        tts_quota.record_usage(student_id, len(gs))

                    # Encode this chunk to WAV in-memory
                    buf = io.BytesIO()
            ...
            logger.exception("Kokoro synthesis stream error for %r using voice %s: %s", text[:60], selected_voice, exc)
