...
def run_test():
    """
    Runs the full parser test suite verifying XML/HTML tags sanitization, markdown rendering,
    JSON format conversions, and edge-case handling.
    """
    ...
    assert "Here are the key points." in full_planned3
    ...
    # Speak tags should not be visible anywhere, show tags are normalized
    assert "SPEAK" not in full_raw6
    assert "speak" not in full_raw6
    assert "<show" in full_raw6
    assert "</show>" in full_raw6
    ...
    # 8. Test JSON response format (speech/display/follow_up) — no raw JSON or followup leakage
    parser8 = RealtimeStreamingParser()
    json_payload = (
        '{"speech": "Don\'t worry, every failure is just a step towards success.", '
        '"display": null, '
        '"follow_up": "Do you have any specific goals for your coding skills?"}'
    )
    raw_output8 = []
    planned_output8 = []
    for i, char in enumerate(json_payload):
        for event in parser8.feed(char):
            raw_output8.append(event["raw"])
            planned_output8.append(event["planned"])
    for event in parser8.finalize():
        raw_output8.append(event["raw"])
        planned_output8.append(event["planned"])
    full_raw8 = "".join(raw_output8)
    full_planned8 = "".join(planned_output8)

    print("\n--- TEST 8: RAW OUTPUT (JSON payload converted) ---")
    print(full_raw8.encode('ascii', errors='replace').decode('ascii'))
    assert "Don't worry, every failure is just a step towards success." in full_raw8
    assert "follow_up" not in full_raw8
    assert "display" not in full_raw8
    assert "{" not in full_raw8
    assert "coding skills" in full_raw8
    assert "coding skills" in full_planned8
